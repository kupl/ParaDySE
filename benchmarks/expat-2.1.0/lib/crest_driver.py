...
class SMTSolver:
	...
	def solve(self, ex, cidx, H1=False, H2=False, H4=False):
		...
		global bscore, bscore_remain, binfo
		...
		binfo = (bscore[pbid], bscore_remain[pbid], cidx, len(dependent_cidxs), 
						sat_cnt.get(pbid, 0), unsat_cnt.get(pbid, 0), try_cnt.get(pbid, 0)-1)
		...
		if result["SMT"] == "SAT":
			...
			#print 'BINFO, UNSAT, 0,', ', '.join(map(str, binfo))
			return result		
...
def print_log(ex, new_bids, divergence, num_added, **kw):
	...
	print "EX_NO:%d NEW:%d covered:%d DIV:%s SAT/UNSAT: %d/%d SMT_HIT: %d" \
					%(ex["ex_no"], len(new_bids), len(covered), 
	...
	"""
	if gex_no != 1:
		print "BINFO, SAT,", len(new_bids), ',', ', '.join(map(str, binfo))
	"""
	print "----------------------------------------------------"
	print ""	
...
def print_final_log():
	...
	print "Iteration:%d covered:%d divergence:%d (%.2f%%) UNSAT:%d SMT_CACHE_HIT:%d CPU_TIME: %.2f WALL_TIME: %.2f" %(
		  gex_no, len(covered), cnt_divergence, 
		  (100.0*float(cnt_divergence)/gex_no), smt_unsat_cnt, smt_cache_hit, time.clock(), time.time() - startTime)
...
def read_info(score=None, dom=None):
	...
	bscore_remain = copy.copy(bscore)
...
def update_coverage(bids):
	...
	global try_cnt
	global dominator
	global bscore_remain
	
	for bid in bids:
		covered[bid] = try_cnt.get(bid, 0)
		if bid in dominator:
			for d in dominator[bid]:
				if d in bscore_remain:
					bscore_remain[d] -= 1
		...
		while True:
			for i in range(BFS_depth+1):
				...
				for ex in row:
					...
					_do_bfs_Hcontext(program, solver, ex, BFS_depth, BFS_width, BFS_list, CONTEXT_depth)
					BFS_width += 1	
			...
			if NOMORE:
				...
				BFS_depth = 0
				CONTEXT_depth += 1
				print "Increase Context", CONTEXT_depth
				tm.incContextDepth()
