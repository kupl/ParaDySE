...
RETSIGTYPE catchsig P((int sig, int code)) __attribute__((crest_skip));
...
char **argv;
{
	...
	char *bufcha;
    char char_c;
	bufcha = malloc(sizeof(char)*30);
       
	for(int i=0; i<29; i++){
	   CREST_unsigned_char(char_c);
	   bufcha[i] = char_c;
	}
    if(bufcha[1]=='f'|| bufcha[1]=='W'|| bufcha[1]=='m' || bufcha[1]=='-' || bufcha[1]=='F'){
    	   bufcha[1]='V';
	}
	bufcha[29]='\0';
	argv[1]=bufcha;
