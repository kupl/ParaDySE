/*
  ...
  Standard API for the Yices SMT Solver (version 1)
...
/**
   ...
   A return code of 0 indicates one of the following errors:
   - \c v is not a proper declaration or not the declaration of a numerical variable
   - \c v has no value assigned in model \c m (typically, this means that \c v does not 
   occur in the asserted constraints).
...
/**
  ...
  character string. The string is returned into a record of the previous type.
   ...
   \sa #yices_free_string
*/
void yices_get_arith_value_as_string(yices_model m, yices_var_decl d, srec_t *r);


/**
  \brief Free the string returned by function #yices_get_arith_value_as_string.

  - If r->flag is 1, then r->str should be a string allocated internally by yices.
    This function frees the string and resets r->flag to 0 and r->str to NULL.

  - If r->flag is 0 and r->str is NULL, the function does nothing.

  \sa #yices_get_arith_value_as_string
...
/**
   ...
   \warning The model cost is computed by #yices_max_sat but not by
   #yices_check.
 */
